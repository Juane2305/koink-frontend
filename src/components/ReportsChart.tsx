    ...
    return (
      <div className="bg-white/95 backdrop-blur-sm p-3 border rounded-lg shadow-xl ring-1 ring-black/5">
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
          {payload[0].name || payload[0].payload.month || payload[0].payload.name}
        </p>
...
export const ReportsChart = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setCategoryData(Object.keys(categoryMap).map(name => ({
        name: name,
        value: categoryMap[name],
      })));

      setMonthlyData(Object.keys(monthlyTotals).map(mIndex => ({
        month: new Date(currentYear, Number(mIndex)).toLocaleString("es-AR", { month: "short" }),
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-10">
      <Card className="overflow-hidden border-none shadow-md bg-white/50 backdrop-blur-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7">
          <div className="space-y-1">
            <CardTitle className="text-xl font-bold tracking-tight">Gastos por Categoría</CardTitle>
      ...
      <Card className="overflow-hidden border-none shadow-md bg-white/50 backdrop-blur-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7">
          <div className="space-y-1">
            <CardTitle className="text-xl font-bold tracking-tight">Evolución Anual</CardTitle>
            <p className="text-sm text-muted-foreground">Gasto mensual en {new Date().getFullYear()}</p>
          </div>
        ...
        <CardContent className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="totalSpent" fill="url(#barGradient)" radius={[6, 6, 0, 0]} barSize={32} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
